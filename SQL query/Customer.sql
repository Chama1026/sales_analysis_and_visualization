...
SELECT 
  ...
  CASE WHEN c.storeid IS NOT NULL THEN s.NAME ELSE Concat(
    p.firstname, 
    Isnull(' ' + p.middlename, ''), 
    ' ', 
    p.lastname
  ) END AS [Customer Name], 
  CASE WHEN c.storeid IS NOT NULL THEN 'Business' ELSE 'Individual' END AS [Customer Type], 
  a.city, 
  CASE WHEN p.emailpromotion = 0 THEN 'Non-subscriber' ELSE 'Subscriber' END AS [Email Promotion], 
